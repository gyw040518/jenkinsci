...
#Downloaded from : https://raw.githubusercontent.com/eea/eea.docker.jenkins/master/slave/start.sh

#wait for Jenkins Master to become available

while [ $(curl -o /dev/null --silent --head --write-out '%{http_code}\n' ${JENKINS_MASTER}) -ne 200 ];
do
    echo "Waiting for ${JENKINS_MASTER} to become available ..."
    sleep 3
done

echo "JENKINS_MASTER is ready : ${JENKINS_MASTER}"
...
echo "Starting Agent with params : "
echo "============================="
echo "/usr/bin/java $JAVA_OPTS -jar /bin/swarm-client.jar -fsroot $JENKINS_HOME/worker/ $PARAMS"
echo "============================="
